...
def database_tst()->None:
    ...
    query_session = session_query(LocalWork, hashed_name__eq=Token.crypt_sha256('Local_A'))

    print('query_model: ', query_model)
    print('query_session: ', query_session)

    # model_update(query_session[0], id="6789")
    session_update(query_session, id="6789")

    query_session = session_query(LocalWork, hashed_name=Token.crypt_sha256('Local_A'))
